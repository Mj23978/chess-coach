				...
				<Table>
					<TableHeader className="sticky top-0">
						{table.getHeaderGroups().map((headerGroup) => (
							...
							>
								{icon && !title ? (
									<TableHead className="w-9 rounded-tl-md"></TableHead>
								) : null}
								{title ? (
									<TableHead
										className={cn(
											"px-0",
											!withoutRadius && "rounded-tl-md rounded-tr-md",
										)}
					...
					<TableBody>
						{isLoading ? (
							...
							table.getRowModel().rows.map((row) => (
								...
								>
									{icon && (
										<TableCell className="w-9 first:pl-3">{icon}</TableCell>
									)}
							...
							<TableRow>
								<TableCell
									...
									className={cn("h-24 text-center", emptyStateHeight)}
								>
			...
			{pagination && (
				<div className="mt-4 flex items-center justify-between px-2">
					<div className="text-muted-foreground text-sm">
						...
						{Math.min(pagination.offset + data.length, pagination.total)} of{" "}
