/**
 * Per-player accuracy % computation. Ported verbatim from chess-kit's
 * `src/lib/engine/helpers/accuracy.ts`.
 *
 * Each move's accuracy derives from the win-percentage drop it caused, via
 * lichess's exponential formula (see lila source link). The player's overall
 * accuracy is the mean of (weighted mean, harmonic mean) over their moves,
 * where weights come from the std-dev of a sliding window of win-percentages
 * (positions where the eval swung a lot matter more for accuracy scoring).
 */
import {
  ceilsNumber,
  getHarmonicMean,
...
import type { Accuracy, PositionEval } from "./types";
...
const getMovesAccuracy = (movesWinPercentage: number[]): number[] =>
  movesWinPercentage.slice(1).map((winPercent, index) => {
    ...
    // Source: https://github.com/lichess-org/lila/blob/a320a93b68dabee862b8093b1b2acdfe132b9966/modules/analyse/src/AccuracyPercent.scala#L44
    const rawAccuracy =
